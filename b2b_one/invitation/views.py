...
from invitations.views import SendInvite, AcceptInvite, SendJSONInvite


Invitation = get_invitation_model()
InviteForm = get_invite_form()

from .forms import CustomInviteForm, EmailFormSet


class CustomInvite(SendInvite):
    form_class = EmailFormSet
    ...
    def form_valid(self, formset):
        ...
        if valid_emails:
            # Process the list of valid email addresses
            self.process_valid_emails(valid_emails)
            print(valid_emails)

        return self.render_to_response(
            self.get_context_data(
                success_message=_("Invitations sent for %(count)d email(s)") % {"count": len(valid_emails)},
            ),
        )
    ...
    #         invite = form.save(email)
    #         invite.inviter = self.request.user
    #         invite.company = self.request.user.company
    #         invite.save()
    #         invite.send_invitation(self.request)
    #     except Exception:
...
def accept_invitation1(invitation, request, signal_sender):
